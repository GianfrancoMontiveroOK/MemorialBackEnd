  ...
  try {
    ...
    for (const a of allocations) {
      ...
      if (a.amount > bal + 0.0001) {
        ...
        freshBal.set(a.period, Math.max(0, bal - a.amount)); // reserva lógica
      }
    }

    // 7) Crear Payment (draft → posted) — AQUÍ recién definimos 'p'
    ...
    await p.save({ session }); // <-- desde acá p existe y se puede usar seguro

    // 8) Ledger (doble partida por total) — usa 'p' ya guardado
    const postedAt = p.postedAt || new Date();
    await LedgerEntry.insertMany(
      [
        {
          paymentId: p._id,
          userId: myUserId, // NUEVO para atribución de caja por usuario
          side: "debit",
          ...
          postedAt,
          dimensions: {
            userId: new mongoose.Types.ObjectId(String(myUserId)), // NUEVO en dimensions
